...
void main()
{
		...
		scanf("%[^\n]",dispstr);
		len=strlen(dispstr);
		if(len>45){strcpy(dispstr,"Too Long String");len=15;}
    ...
    /*	fp=fopen("text.txt","w");
	for(i=0;i<cursorcount;i++)                // Writing to file
	...
	{       st=dispreg[i];
		if(i%26>20&&st==0)getch();
		printf("%2x=\t|",st);
		for(j=0;j<8;j++,st*=2,st%=256)
		{	if(st/128==1)printf("*");
			else printf(" ");
		}
		printf(" |\n");
